...
Why I split them:  My intuition was that nano banana is good at generating or editing images given it is given clear cut instructions. We shouldn't depend on it to reason and then generate. And by visual inspection, the outputs have been much better.

Nano banana prompting technique:  For the prompt generation passed the [Nano Banana Best Practices as context](https://ai.google.dev/gemini-api/docs/image-generation#best-practices)
